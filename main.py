...
BOT_VERSION = "v10.8 DANGER PRIORITY FIX"
...
def market_risk_level(ctx):
    ...
    # v10.8: danger имеет приоритет над safe-caution.
    # BTC почти -3% + экстремальный страх — это уже опасный рынок, даже если новости не полностью красные.
    if btc_change <= -2.5 and fg_value <= 15:
        return "danger"

    # BTC падает сильнее -2.3%, страх высокий и новости негативные — опасный рынок.
    if btc_change <= -2.3 and fg_value <= 20 and news_score <= -6:
        return "danger"
...
def v106_safe_caution(ctx):
    """
    v10.8:
    Safe-caution — только пограничный режим.
    Danger всегда выше safe-caution.
    """
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v10.8: danger priority выше safe-caution; при BTC около -3%, страхе и негативных новостях показывается 🔴 опасный рынок"
    )
