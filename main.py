...
BOT_VERSION = "v10.3 LEARNING VISIBILITY FIX"
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v10.3: /learning показывает открытые наблюдения, текущий результат, проверки 1ч/6ч/24ч/48ч и время до закрытия"
    )
