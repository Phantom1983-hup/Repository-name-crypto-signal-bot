...
BOT_VERSION = "v8.6 QUIET ALERT MODE"
...
def macro_action_hint(ctx):
    ...
    if score <= -4:
        return "Вывод: быстрые BUY по альтам ограничить; BTC/ETH только после стабилизации и малыми частями."
    if score < 8:
...
def save_signal_history(items):
    ...
    for c in items:
        ...
        # Не плодим одинаковые записи по одной монете чаще одного раза в 45 минут.
        last_open = None
        ...
        if last_open and now - float(last_open.get("time", 0)) < 45 * 60:
            continue
...
def compact_learning_text():
    ...
    if isinstance(data, dict):
        ...
        if isinstance(open_items, dict) and len(open_items):
            return f"📚 Самообучение: открытых сигналов {len(open_items)}, ждём 24/48ч результаты\n"
...
def format_single_coin_report(c):
    ...
    if c.get("_btc_core_watch"):
        ...
        if c.get("symbol") == "BTC":
            text += "Итог: BTC перепродан, но вход только после стабилизации. Не ловить нож."
        else:
...
def help_text():
    return (
        ...
        "🟡 НАБЛЮДАТЬ — пока не покупать\n🟦 СРЕДНЕСРОЧНЫЙ НАБОР — сначала стабилизация, потом только малая часть\n"
        "🟠 РИСКОВАННЫЙ ИМПУЛЬС — не догонять, лучше ждать откат\n"
