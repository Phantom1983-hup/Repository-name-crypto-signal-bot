...
BOT_VERSION = "v5.6.1 SAFE NEUTRAL PATCH"
...
def apply_safe_neutral_patch(c):
    """
    Безопасный патч:
    не меняет adjusted_score(),
    не ломает сортировки,
    только не даёт BTC/ETH/SOL выглядеть как 0/100,
    если это просто слабый рынок без экстремального обвала.
    """
    if not c:
        return c
    ...
    floor = 25 if symbol in ["BTC", "ETH"] else 20

    if c.get("score", 0) < floor:
        c["score"] = floor

    # Если потенциала почти нет, это не "ужасная монета", а просто отсутствие сигнала.
...
def neutral_explanation_text(c):
    ...
    return (
        ...
        "Это не аварийная ситуация и не сигнал к покупке. "
        "Лучше ждать рост объёма, улучшение импульса и подтверждение рынка.\n\n"
    )
