...
BOT_VERSION = "v11.2 FAST GITHUB STORAGE FIX"
...
def weekday_report():
    ...
    sync_github_storage_now([WEEKDAY_FILE, CHAT_ID_FILE])
...
def learning_report():
    ...
    sync_github_storage_now([RESULTS_FILE, CHAT_ID_FILE])
...
def get_signal():
    try:
        ...
        if not buy and not accum and not watch and not pumps and not aggressive and not speculative and not early_text and not late_pumps:
            ...
            sync_github_storage_now([RESULTS_FILE, HISTORY_FILE, CHAT_ID_FILE])
...
def should_send_pump(items):
    ...
    if allowed:
        ...
        sync_github_storage_now([PUMP_FILE, CHAT_ID_FILE])
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v11.2: быстрый GitHub storage без зависания /signal; синхронизация json пачкой после команды"
    )
