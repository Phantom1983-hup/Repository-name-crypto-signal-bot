...
BOT_VERSION = "v10.2 BTC FILTER FALLBACK"
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v10.2: BTC-фильтр не отдаёт 0.00% при сбое свечей; если ticker работает, риск считается по реальному BTC 24ч"
    )
