...
BOT_VERSION = "v5.6 NEUTRAL QUALITY FIX"
...
def format_signal_item(i, c):
    ...
    return (
        ...
        f"{('Скорректированная оценка: ' + str(display_adjusted_score(c)) + '/100\n') if display_adjusted_score(c) != c.get('score') else ''}"
        f"Уверенность сигнала: {confidence}%\n\n"
        f"{neutral_status_text(c)}"
        f"Шансы на 24ч:\n"
        f"+5% → ~{c['chance_5']}%\n"
        f"+10% → ~{c['chance_10']}%\n"
        f"+15% → ~{c['chance_15']}%\n\n"
...
def buyer_strength_score(c):
    ...
    if is_core_quality_asset(c) and not is_extreme_bad_condition(c):
        score = max(score, 15)
...
def learning_stats_text():
    ...
    text += "Бот будет мягко подстраивать вероятность +5% по этой статистике.\n\n"
    return text
...
def neutral_quality_floor(c):
    """
    Качественный актив в слабом рынке не должен получать 0/100,
    если это не обвал, не экстремальный перегрев и не полное исчезновение объёма.
    Это не разрешение покупать, а защита от неверного вывода "монета ужасная".
    """
    if not is_core_quality_asset(c):
        return c

    if is_extreme_bad_condition(c):
        return c

    c = dict(c)

    floor = 25 if c.get("symbol") in ["BTC", "ETH"] else 20

    if c.get("score", 0) < floor:
        c["score"] = floor

    adj = display_adjusted_score(c)
    if adj < floor:
        # Не меняем формулу adjusted_score глобально, но не даём качественному активу падать в ноль в нейтральном ожидании.
        c["_quality_floor_adjusted"] = floor
...
def display_display_adjusted_score(c):
    return int(c.get("_quality_floor_adjusted", adjusted_score(c)))

def neutral_status_text(c):
    if "НЕТ СИГНАЛА" in c.get("verdict", ""):
        return (
            "⚪ НЕТ СИГНАЛА\n\n"
            f"{c.get('symbol')} сейчас без нормального импульса. "
            "Это не аварийный сигнал, а режим ожидания: покупать рано, "
            "нужно дождаться роста объёма и улучшения импульса.\n\n"
        )
    return ""
...
def speculative_watch_text(items):
    ...
    for i, c in enumerate(items[:3], 1):
        text += (
            f"{i}. {c['symbol']} — adjusted {display_adjusted_score(c)}/100, raw {c.get('score')}/100\n"
...
def best_watch_candidates(analyzed):
    ...
    quality_pool = [
        x for x in analyzed
        if (
            display_display_adjusted_score(x) >= 25
...
def action_plan_from_analyzed(analyzed):
    ...
    selected = [x for x in selected if x.get("score", 0) >= 20 or display_adjusted_score(x) >= 20]

    if not selected:
        selected = sorted(
            [x for x in analyzed if x.get("score", 0) >= 20 or display_adjusted_score(x) >= 20],
            key=lambda x: (display_adjusted_score(x), x.get("score", 0), x.get("chance_5", 0)),
            reverse=True
        )[:4]
    else:
        selected = selected[:4]
...
def early_candidates_from_analyzed(analyzed, exclude_symbols=None):
    ...
    items = [
        ...
        if (
            ...
            and x.get("action") != "BUY"
            and 30 <= display_adjusted_score(x) < 45
    ...
    for i, c in enumerate(items, 1):
        text += (
            f"{i}. {c['symbol']} — adjusted {display_adjusted_score(c)}/100 "
...
def market_counts_text(buy, watch, aggressive, speculative, early_text):
    early_count = 0
    if early_text:
    ...
    return (
        "📊 Срез рынка:\n"
        f"BUY: {len(buy)} | WATCH: {len(watch)} | Осторожные: {len(aggressive)} | "
...
def best_current_candidate_text(analyzed):
    ...
    c = sorted(
        candidates,
        key=lambda x: (display_adjusted_score(x), asset_quality_rank(x), reward_risk_ratio(x)),
        reverse=True
    )[0]

    return (
        "⭐ Лучший кандидат сейчас:\n\n"
        f"{c['symbol']} — adjusted {display_adjusted_score(c)}/100, raw {c.get('score')}/100\n"
...
def get_signal():
    try:
        ...
        for x in analyzed:
            if display_adjusted_score(x) < 45 and x.get("action") in ["WATCH", "PUMP"]:
                x["action"] = "SKIP"
        ...
        watch = sorted(
            [
                ...
                if (
                    ...
                    and x["chance_5"] >= 35
                    and x["high"] >= 5
                    and display_adjusted_score(x) >= 45
                    and x["change_24"] < 25
                )
            ],
            key=lambda x: (
        ...
        aggressive = sorted(
            ...
            key=lambda x: (display_adjusted_score(x), asset_quality_rank(x), reward_risk_ratio(x), x["chance_5"]),
            reverse=True
        )[:3]
        ...
        speculative_watch = sorted(
            [
                ...
                if (
                    ...
                    and display_display_adjusted_score(x) >= 30
            ...
            key=lambda x: (display_adjusted_score(x), x.get("chance_5", 0), x.get("high", 0)),
            reverse=True
        )[:3]
        ...
        if not buy and not watch and not pumps and not aggressive and not speculative and not early_text and not late_pumps:
            plan = action_plan_from_analyzed(analyzed)
            return f"🚀 ALEX EDGE ULTRA {BOT_VERSION}\n\nСейчас нет нормальных идей для покупки.\n\n" + plan

        save_signal_history(buy + watch + pumps + aggressive + speculative)
        ...
        text += f"{ctx['geo_text']}\n\n"
        text += learning_stats_text()
        text += market_counts_text(buy, watch, aggressive, speculative, early_text)
        if "speculative_watch" in locals() and speculative_watch:
            text += f"Спекулятивных идей отдельно: {len(speculative_watch)}\n\n"
        ...
        if buy:
            ...
            text += best_current_candidate_text(analyzed)
            text += action_plan_from_analyzed(analyzed)
            # speculative watch is printed later once, to avoid duplicates
        ...
        if aggressive:
            text += "🟠 МОЖНО МАЛЫМ ОБЪЁМОМ / РИСКОВАННЫЙ ВХОД:\n\n"
            for i, c in enumerate(aggressive, 1):
                text += format_signal_item(i, c)

        if speculative:
            text += "🟣 СПЕКУЛЯТИВНЫЕ ИДЕИ / ТОЛЬКО МИКРОПОЗИЦИЯ:\n\n"
            for i, c in enumerate(speculative, 1):
                text += format_signal_item(i, c)

        if "early_text" in locals() and early_text:
            text += early_text

        if speculative_watch:
            if "speculative_watch" in locals() and speculative_watch:
                text += speculative_watch_text(speculative_watch)
