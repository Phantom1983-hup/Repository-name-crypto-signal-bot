...
BOT_VERSION = "v11.1 FREE GITHUB STORAGE"
...
def save_chat_id(chat_id):
    ...
    if github_storage_enabled():
        try:
            github_storage_put_raw(CHAT_ID_FILE, text.encode("utf-8"), "update chat_id")
        except Exception:
            pass
...
def load_chat_id():
    ...
    try:
        return open(CHAT_ID_FILE, encoding="utf-8").read().strip()
    except Exception:
        return None
...
def save_json(path, data):
    ...
    if github_storage_enabled():
        try:
            ...
            github_storage_put_raw(path, content, f"update {os.path.basename(str(path))}")
        except Exception:
            # Telegram-бот не должен падать, если GitHub временно недоступен.
            pass
...
def help_text():
    return (
        ...
        "/storage, /backup, /weekday, /admin_update, /rollback\n"
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v11.1: бесплатное GitHub storage для json-обучения, /backup, /storage, /weekday и admin auto deploy"
    )
