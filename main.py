...
BOT_VERSION = "v5.7 TRUE NEUTRAL MODE"
...
def apply_safe_neutral_patch(c):
    """
    v5.7 TRUE NEUTRAL MODE:
    Отличаем плохой сигнал от отсутствия сигнала.
    BTC/ETH/SOL/TAO могут быть слабыми, но это не значит 0/100.
    """
    if not c:
        return c
    ...
    neutral_market = (
        ...
        and c.get("volume_trend", 1) >= 0.45
        and c.get("high", 0) <= 2.5
    )
    ...
    if neutral_market:
        ...
        c["target_low"] = c.get("price", 0) * (1 + c["low"] / 100)
        c["target_high"] = c.get("price", 0) * (1 + c["high"] / 100)
        c["verdict"] = "⚪ НЕТ СИГНАЛА"
        c["entry_zone"] = "нейтрально: ждать рост объёма и подтверждение импульса"

        if "режим ожидания, а не плохой сигнал" not in c.get("minus", []):
            c.setdefault("minus", []).append("режим ожидания, а не плохой сигнал")
