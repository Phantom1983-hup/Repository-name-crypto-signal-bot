...
def needs_aggressive_signal(c):
    ...
    if c.get("ctx", {}).get("btc_mod", 0) < -12:
        return False
    if c.get("volume_trend", 1) < 0.7:
        return False
    if c.get("high", 0) < 4:
        return False
    if abs(c.get("downside", 0)) > c.get("high", 0) * 1.7:
        return False
...
def best_watch_candidates(analyzed):
    eligible = [x for x in analyzed if x.get("score", 0) >= 40]
    ...
    for i, c in enumerate(top, 1):
        ...
        txt += f"{i}. {c['symbol']} — {c['score']}/100 | до BUY примерно {need} баллов\n"
    return txt + "\n"
...
def get_signal():
    try:
        ...
        aggressive = sorted(
            ...
            key=lambda x: (asset_quality_rank(x), x["score"], x["chance_5"]),
            reverse=True
        )[:3]
        ...
        if not buy and not watch and not pumps and not aggressive and not speculative and not late_pumps:
