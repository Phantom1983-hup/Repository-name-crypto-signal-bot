...
def coin_profile(asset, volume):
    if asset == "BTC":
        return "BTC / низкая волатильность", 0.5, 3.0, True
    if asset == "ETH":
        return "ETH / средняя волатильность", 0.8, 4.0, True
    if asset in QUALITY_ASSETS:
        return "крупный альт", 1.5, 7.0, True
    if volume >= 30_000_000:
        return "ликвидный альт", 2.0, 8.0, False
    return "спекулятивный альт", 2.0, 10.0, False
...
def trend_score(symbol, interval):
    ...
    if len(closes) < 30:
        return {
            "score": 0, "rsi": 0, "macd": 0, "volume_x": 1,
            "support": 0, "resistance": 0, "atr_pct": 0,
            "continuation": 0, "continuation_reasons": [],
            "breakout_confirmed": False,
            "divergence": "нет данных", "divergence_score": 0
        }
    last = closes[-1]
    r = rsi(closes)
...
def get_news_risk():
    try:
        ...
        words = {
            "iran": 2, "hormuz": 3, "strait": 2, "oil": 2,
            "war": 3, "attack": 2, "fed": 2, "inflation": 2,
            "tariff": 2, "sanction": 2, "trump": 1, "missile": 3
        }
        score = 0
        found = []
...
def market_context():
    ...
    if "🔴" in geo_label:
        state = "🟡 осторожный из-за геориска"
    elif total_mod >= 8:
        state = "🟢 благоприятный"
    elif total_mod >= -5:
...
def apply_score_caps(score, asset, profile, is_quality, change, rsi_value, volume_x, risks):
    cap = 100
    if volume_x < 1.5 and not is_quality:
        cap = min(cap, 75)
        risks.append("спекулятивный альт без сильного объёма")
    if volume_x < 0.8:
        cap = min(cap, 78)
        risks.append("рост без подтверждения объёмом")
    if volume_x < 0.5:
        cap = min(cap, 72)
    if rsi_value > 78 and volume_x < 1.5:
        cap = min(cap, 75)
        risks.append("RSI высокий без сильного объёма")
    if rsi_value > 86:
        cap = min(cap, 70)
        risks.append("RSI сильно перегрет")
    if rsi_value > 92:
        cap = min(cap, 62)
        risks.append("RSI экстремально перегрет")
    if not is_quality and change > 8:
        cap = min(cap, 78)
        risks.append("спекулятивный импульс")
    if not is_quality and change > 12:
        cap = min(cap, 72)
        risks.append("высокий риск отката")
    if not is_quality and change > 20:
        cap = min(cap, 62)
        risks.append("памп-зона")
    return min(score, cap), risks
def potential_range(asset, score, change, rsi_value, volume_x, atr_pct, macro_mod, resistance_gap, continuation, breakout_confirmed, divergence_score, volume):
    profile, base_low, base_high, is_quality = coin_profile(asset, volume)
    if score >= 90:
        ...
        low, high = -1.5, 2.0
    if atr_pct > 0:
        high = min(high, max(1.5, atr_pct * 2.3))
    if continuation >= 25:
        low += 0.5
        high += 0.8
    ...
    if rsi_value > 86:
        low -= 1.0
        high -= 1.5
    if change > 20 and not is_quality:
        low = min(low, -4)
        high = min(high, 2.5)
    ...
    return low, high, profile, is_quality
def analyze_coin(symbol):
    ...
    ctx = market_context()
    profile, _, _, is_quality = coin_profile(asset, volume)
    score = 0
    reasons = []
    risks = []
    if change > 0:
    ...
    if volume > 5_000_000 and is_quality:
        score += 15
    ...
    score, risks = apply_score_caps(
        score, asset, profile, is_quality, change, h1["rsi"], h1["volume_x"], risks
    )
    support = h1["support"]
    resistance = h1["resistance"]
    resistance_gap = ((resistance / price) - 1) * 100 if resistance > price else 0
    pot_low, pot_high, profile, is_quality = potential_range(
        asset, score, change, h1["rsi"], h1["volume_x"], h1["atr_pct"],
        ctx["macro_mod"], resistance_gap, h1["continuation"],
        h1["breakout_confirmed"], h1["divergence_score"], volume
    )
    ...
    if not is_quality and score >= 65:
        verdict = "🟡 импульсный рискованный сигнал"
        probability = min(64, 42 + int(score / 6))
    elif score >= 80:
...
def get_signal():
    try:
        ...
        preselected = sorted(usdt, key=lambda x: x["preliminary"], reverse=True)[:25]
        analyzed = []
        for item in preselected:
            try:
                ...
                continue
        quality = sorted(
            [x for x in analyzed if x["is_quality"]],
            key=lambda x: x["score"],
            reverse=True
        )[:3]
        speculative = sorted(
            [x for x in analyzed if not x["is_quality"] and x["score"] >= 55],
            key=lambda x: x["score"],
            reverse=True
        )[:3]
        all_for_history = quality + speculative
        if all_for_history:
            save_signal_history(all_for_history)
        if not quality and not speculative:
            return "Сейчас качественных сигналов нет."
        ctx = (quality[0] if quality else speculative[0])["ctx"]
        dom_text = "нет данных"
        ...
        text += "\n\n"
        if quality:
            text += "🥇 Качественные сигналы:\n\n"
            for i, c in enumerate(quality, 1):
                text += format_coin_block(i, c)
        if speculative:
            text += "⚡ Спекулятивные импульсы:\n\n"
            for i, c in enumerate(speculative, 1):
                text += format_coin_block(i, c)
        text += "⚠️ Спекулятивные импульсы — это не качественные входы. Если BTC резко развернётся, прогноз отменяется."
        return text
...
def help_text():
    return (
        ...
        "/help — помощь\n\n"
        "В /signal теперь отдельно: качественные сигналы и спекулятивные импульсы."
    )
def main():
    last_update = None
