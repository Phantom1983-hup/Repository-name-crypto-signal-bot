...
BOT_VERSION = "v11.0 PERSISTENT STORAGE AUTO DEPLOY"
...
def storage_report():
    lines = [
        ...
        "",
        "Для Render Persistent Disk: mount path /var/data и env DATA_DIR=/var/data."
    ]
    return "\n".join(lines)
...
def admin_help():
    return (
        ...
        "DATA_DIR — /var/data для Persistent Disk"
    )
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v11.0: DATA_DIR persistent storage, /backup, /storage, /weekday и admin auto deploy через GitHub/Render"
    )
    ...
    while True:
        try:
            ...
            for item in updates.get("result", []):
                ...
                if text == "/start":
                    send_message(chat_id, "✅ Бот работает\nДобавлены persistent storage, /backup, /weekday и admin auto deploy.\n\n" + help_text())
