...
BOT_VERSION = "v4.3 REJECTION LOGIC"
...
def format_signal_item(i, c):
    ...
    return (
        ...
        f"Уверенность сигнала: {confidence_level(c)}%\n\n"
        f"Шансы на 24ч:\n"
        f"+5% → ~{c['chance_5']}%\n"
        f"+10% → ~{c['chance_10']}%\n"
        f"+15% → ~{c['chance_15']}%\n\n"
...
def worth_it_text(c):
    ...
    return (
        ...
        f"{verdict}\n\n"
        f"Потенциал роста: до +{high:.1f}%\n"
        f"Риск снижения: около -{stop:.1f}%\n\n"
        f"{text}\n"
    )
...
def best_watch_candidates(analyzed):
    if not analyzed:
        return ""
    top = sorted(analyzed, key=lambda x: x.get("score",0), reverse=True)[:5]
...
def get_signal():
    try:
        ...
        if pumps:
            text += "🟠 ЖДАТЬ ОТКАТ / ПРОДОЛЖЕНИЕ ТРЕНДА:\n\n"
