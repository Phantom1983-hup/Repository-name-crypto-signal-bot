...
BOT_VERSION = "v9.7 MARKET RISK ENGINE"
...
def market_risk_level(ctx):
    """
    v9.7:
    Новостной score может быть смешанным, но если BTC падает -4% и страх 15,
    режим не может быть "обычный".
    """
...
def macro_action_hint(ctx):
    ...
    if level == "danger":
        return (
            "Вывод: рынок опасный. Быстрые BUY запрещены; BTC/ETH только после стабилизации, "
            "альты только наблюдать. Новостной фон может быть смешанным, но BTC/страх сейчас важнее."
        )

    if level == "caution":
        return (
            "Вывод: осторожный режим. Быстрые BUY ограничить; вход только после подтверждения объёмом "
            "и стабилизации BTC."
        )

    if level == "positive":
        return "Вывод: фон помогает, можно активнее искать BUY, но всё равно вход частями."

    return "Вывод: обычный режим только по новостям; входы всё равно ждать по цене и объёму."
...
def learning_report():
    ...
    if not closed:
        return (
            ...
            f"Закрытых 48ч результатов пока нет.\n\n"
            f"Бот уже записывает сигналы и будет проверять их через 1ч / 6ч / 24ч / 48ч."
        )
    ...
    for rec in closed:
        a = rec.get("asset", "?")
        by_asset.setdefault(a, {"n": 0, "success": 0, "bad": 0})
        by_asset[a]["n"] += 1
    ...
    ranked = sorted(
        by_asset.items(),
        key=lambda kv: (kv[1]["success"] - kv[1]["bad"], kv[1]["n"]),
        reverse=True
    )[:5]
    ...
    if ranked:
        ...
        for asset, s in ranked:
            text += f"• {asset}: +{s['success']} / -{s['bad']} / всего {s['n']}\n"
...
def compact_reason(c):
    ...
    if "СРЕДНЕСРОЧНЫЙ" in c.get("verdict", ""):
        ...
        if c.get("_cautious_accum"):
            return "score ниже 70, ждать остановку падения"
...
def format_fast_alert(items):
    ...
    quality = [x for x in items if x.get("kind") == "quality"]
    speculative = [x for x in items if x.get("kind") == "speculative"]

    text = f"⚡ ALEX FAST ALERT {BOT_VERSION}\n\n"
    text += "Быстрые импульсы по рынку. Это не спокойный сигнал и не повод догонять свечу.\n\n"
    ...
    if quality:
        ...
        for i, c in enumerate(quality[:3], 1):
            ...
            text += (
                ...
                f"Действие: наблюдать, вход только после отката/подтверждения. Риск: {risk}.\n\n"
            )
...
def get_fast_pumps():
    """
    v8.6 SAFE ALERTS / ручная команда /alerts:
    - алерты делятся на качественные и спекулятивные;
    - неизвестные монеты не получают score 90/100;
    - при плохом фоне мелкие монеты не выглядят как сильный сигнал.
    """
    try:
        found = []
        ctx = market_context()
        ...
        for t in pairs:
            ...
            try:
                ...
                # Для качественных активов можно ловить более мягкий импульс.
                # Для неизвестных — только сильное движение, но всё равно без "сильного" score.
                if kind == "quality":
                    impulse = (
                        (fast_move >= 1.0 and vol_power >= 1.3)
                        or (fast_move >= 0.7 and vol_power >= 2.0)
                        or (fast_move >= 1.8 and vol_power >= 1.1)
                    )
                else:
                    impulse = (
...
def should_send_pump(items):
    ...
    for c in items:
        ...
        # В плохом фоне пушим только действительно сильные качественные импульсы.
        if macro_mod <= -8 or btc_change < 0:
            if c.get("score", 0) < 72:
                continue
            if c.get("fast_move", 0) < 1.5:
                continue
            if c.get("vol_power", 0) < 1.8:
                continue
        else:
            if c.get("score", 0) < 68:
                continue
            if c.get("fast_move", 0) < 1.0:
                continue
            if c.get("vol_power", 0) < 1.4:
                continue
...
def format_single_coin_report(c):
    ...
    text = (
        ...
        f"Цель: ${c.get('target_low', 0):.6g}…${c.get('target_high', 0):.6g}\n\n"
...
def market_status():
    ...
    risk_text = {
        ...
        "caution": "🟡 осторожно",
        "neutral": "🟡 нейтрально",
        "positive": "🟢 фон помогает"
    }.get(ctx.get("risk_level"), "🟡 нейтрально")

    text = (
        f"🌍 Обзор рынка\n"
        f"Версия: {BOT_VERSION}\n\n"
        f"Рынок: {ctx['state']}\n"
        f"Риск-режим: {risk_text} | общий score: {ctx.get('market_mod', 0):+d}\n"
    ...
    text += (
        f"News score: {ctx.get('macro_mod', 0):+d}\n"
        f"{ctx['macro_text']}\n"
        f"⚠️ Оценка по заголовкам: возможны ошибки источников и формулировок.\n\n"
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v9.7: вывод рынка учитывает BTC + страх + новости"
    )
