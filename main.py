...
BOT_VERSION = "v5.0 ADAPTIVE SIGNALS"
...
def best_watch_candidates(analyzed):
    ...
    def rank(x):
        quality = asset_quality_rank(x)
        market_bad = x.get("ctx", {}).get("btc_mod", 0) < 0
        speculative_penalty = 25 if market_bad and quality < 50 else 0
        return (
            quality - speculative_penalty,
            x.get("score", 0),
            x.get("chance_5", 0)
        )
