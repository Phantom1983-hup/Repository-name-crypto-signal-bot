...
BOT_VERSION = "v5.4 EARLY CANDIDATES"
...
def best_watch_candidates(analyzed):
    eligible = [
        x for x in analyzed
        if adjusted_score(x) >= 30 and x.get("action") != "BUY"
    ]

    if not eligible:
        return "🟦 ЛУЧШИЕ КАНДИДАТЫ НА НАБЛЮДЕНИЕ:\n\nСейчас качественных кандидатов нет.\n\n"
    ...
    top = sorted(eligible, key=rank, reverse=True)[:5]
