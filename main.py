...
BOT_VERSION = "v9.0 COIN BUTTON LIST"
...
def coin_search_prompt():
    return (
        ...
        "Можно также просто написать тикер сообщением: ETH, SUI, LINK, TON.\n"
        ...
        try:
            ...
            for item in updates.get("result", []):
                ...
                if raw_text in MANUAL_COIN_BUTTONS:
                    ...
                    send_message(
                        chat_id,
                        "✍️ Введи тикер монеты обычным сообщением. Например: ETH, SUI, LINK, TON.",
                        reply_markup=keyboard()
                    )
                    continue

                if chat_id in coin_search_waiting and not text.startswith("/"):
                    coin = normalize_coin_input(raw_text)

                    if coin in POPULAR_COINS or (coin and len(coin) >= 2 and len(coin) <= 12):
                        coin_search_waiting.discard(chat_id)
                        send_message(chat_id, f"⏳ Анализирую {coin}, подожди 10–30 секунд...")
                ...
                if raw_text in POPULAR_COINS:
                    send_message(chat_id, f"⏳ Анализирую {raw_text}, подожди 10–30 секунд...")
                    send_message(chat_id, single_analysis(f"{raw_text}-USDT"))
                    continue
                ...
                if text == "/start":
                    send_message(chat_id, "✅ Бот работает\nНижнее меню обновлено: добавлен список монет кнопками.\n\n" + help_text())
                    ...
                    if len(parts) < 2:
                        ...
                        coin = normalize_coin_input(parts[1])
                        send_message(chat_id, f"⏳ Анализирую {coin}, подожди 10–30 секунд...")
                        send_message(chat_id, single_analysis(f"{coin}-USDT"))
