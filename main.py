...
BOT_VERSION = "v10.7 HARD SAFE CAUTION FIX"
...
def v106_safe_caution(ctx):
    """
    v10.6:
    Пограничный режим: страх экстремальный, BTC почти -2% и мешает рынку.
    Это ещё может быть не полноценный danger, но это уже не обычный осторожный рынок.
    """
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v10.7: hard safe-caution при страхе 14–15 и BTC <= -1.6%; BTC/ETH без входа, альты после стабилизации BTC"
    )
