...
BOT_VERSION = "v7.0 RED MARKET ACCUMULATION"
...
def get_signal():
    try:
        ...
        text += market_counts_text(buy, accum, watch, aggressive, speculative, early_text)
        ...
        if buy:
            ...
            text += "🟢 Быстрых покупок сейчас нет.\n\n"
        ...
        if accum:
            text += "🟦 СРЕДНЕСРОЧНЫЙ НАБОР / КРАСНЫЙ РЫНОК:\n\n"
            for i, c in enumerate(accum, 1):
                text += format_signal_item(i, c)
                text += v7_accumulation_plan_text(c) + "\n"
...
def help_text():
    return (
        ...
        "🟡 НАБЛЮДАТЬ — пока не покупать\n🟦 СРЕДНЕСРОЧНЫЙ НАБОР — частичный вход на красном рынке\n"
