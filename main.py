...
BOT_VERSION = "v9.6 NEWS QUALITY + SENTIMENT FIX"
...
def market_context():
    ...
    # v9.6:
    # Если BTC падает сильнее -3% и macro очень плохой, состояние рынка не может быть "осторожным".
    if macro_mod <= -12 and btc_change <= -3:
        state = "🔴 рынок рискованный"
    elif macro_mod <= -16 and btc_change <= -1:
        state = "🔴 рынок рискованный"
    elif total >= 8:
        state = "🟢 рынок помогает росту"
    elif total >= -5 and not (macro_mod <= -12):
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес"
    )
