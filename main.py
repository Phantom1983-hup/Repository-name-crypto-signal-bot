...
BOT_VERSION = "v9.5 LIVE NEWS MACRO"
...
def headline_word_score(items, words):
    """
    Считаем не весь RSS-текст подряд, а отдельные свежие заголовки.
    Одно слово учитывается максимум один раз на заголовок.
    """
    score = 0
...
def matched_headlines(items, words, limit=2):
    rows = []
    ...
    for item in items:
        ...
        for word, weight in words.items():
            if word in text:
                ...
                rows.append((weight, title))
                break

    rows = sorted(rows, key=lambda x: x[0], reverse=True)
    return [x[1] for x in rows[:limit]]

def news_category_score(items, positive_words, risk_words, pos_cap=8, risk_cap=-10):
    positive = headline_word_score(items, positive_words)
    risk = headline_word_score(items, risk_words)
    raw = positive - risk
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по свежим RSS-заголовкам каждые 15 минут"
    )
