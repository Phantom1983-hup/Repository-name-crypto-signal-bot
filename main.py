...
BOT_VERSION = "v6.0 SINGLE SCORE ENGINE"
...
def v6_master_score(c):
    ...
    if group == "speculative" and (btc_mod < 0 or market_mod < -5):
        score -= 12
...
def v6_apply_single_score_engine(c):
    ...
    После него все ключевые поля согласованы:
    score / chance / high / verdict / action / targets.
    """
    if not c:
        return c
    ...
    # Сценарий не должен быть 0%, если шанс движения всё же есть.
    if c.get("chance_5", 0) >= 10:
        c["high"] = max(c.get("high", 0), 1.5)

    # Решение от единого score
    if neutral:
    ...
    elif ms >= 78 and c.get("high", 0) >= 5 and rr >= 1.1:
        c["score"] = ms
        c["chance_5"] = max(c.get("chance_5", 0), 60)
        c["chance_10"] = max(c.get("chance_10", 0), 15)
        c["verdict"] = "🟢 ПОКУПКА / цель +5%"
        c["action"] = "BUY"
        c["entry_zone"] = "можно рассмотреть частичный вход, не после резкой свечи"
    elif ms >= 62 and c.get("high", 0) >= 4 and rr >= 0.8:
        c["score"] = ms
        c["chance_5"] = max(c.get("chance_5", 0), 40)
        ...
        c["score"] = ms
        c["chance_5"] = max(c.get("chance_5", 0), 30)
        c["verdict"] = "🟡 НАБЛЮДАТЬ"
        c["action"] = "WATCH"
    ...
    # Для красных сигналов не обещаем высокий сценарий.
    if "НЕ ПОКУПАТЬ" in c.get("verdict", ""):
        c["high"] = min(c.get("high", 0), 1.5)

    # Для neutral и watch сценарий должен быть читаемый.
    if "НЕТ СИГНАЛА" in c.get("verdict", ""):
        c["high"] = max(c.get("high", 0), 1.5)

    # Обновляем цели после правки low/high.
...
def worth_it_text(c):
    ...
    if "НЕТ СИГНАЛА" in verdict_text:
        verdict = "🟡 Пока рано"
