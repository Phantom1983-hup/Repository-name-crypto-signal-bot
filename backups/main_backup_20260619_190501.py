...
BOT_VERSION = "v12.2 SIGNAL FAILSAFE STATUS"
...
def help_text():
    return (
        ...
        "/storage, /backup, /weekday, /flush, /repair_learning, /signal_unlock, /signal_status, /learning_sync, /sync_storage, /admin_update, /rollback\n"
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v12.2: /signal сразу присылает безопасный предварительный отчёт + /signal_status"
    )
            ...
            for item in updates.get("result", []):
                ...
                if text == "/start":
                    ...
                    if not ok:
                        ...
                        send_message(
                            chat_id,
                            "⏳ /signal запущен в фоне. Сейчас пришлю предварительный безопасный режим, затем полный отчёт."
                        )
