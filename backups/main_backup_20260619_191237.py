...
BOT_VERSION = "v12.3 SAFE SIGNAL SPLIT"
...
def quick_signal_report():
    ...
    for i, sym in enumerate(SIGNAL_QUICK_COINS, start=1):
        ...
        price_text = f"${price:,.4g}".replace(",", " ") if price else "цена недоступна"
    ...
    return (
        ...
        + "\n".join(rows)
        + "\n\n"
        "Полный скан 35 монет: /signal_full\n"
...
def help_text():
    return (
        ...
        "📰 Новости: ФРС/геополитика/крипто обновляются по RSS-заголовкам каждые 15 минут\n🧠 v9.6: deal/ceasefire/end war/reopen Hormuz считаются деэскалацией, слабые источники получают меньший вес; v12.3: /signal быстрый и безопасный; полный тяжёлый скан вынесен в /signal_full"
    )
            ...
            for item in updates.get("result", []):
                ...
                if text == "/start":
                    ...
                    if not ok:
                        ...
                        send_message(
                            chat_id,
                            "⏳ Полный скан 35 монет запущен в фоне. Сначала у тебя уже есть быстрый /signal; полный отчёт придёт отдельно."
                        )
